...
// BOJ11651, Sorting Problem.
// N points above the two-dimensional plane are given. 
// The problem of aligning coordinates with the order in which the coordinates y are increased, 
// and if the coordinates x are equal, the order in which the x-coordinates are increased.
// I solved the problem through a merge sort
public class BOJ11651 {
	ArrayList<Point> mergeList = new ArrayList<Point>();
	
	public BOJ11651() throws IOException {
	...
	public void merge(ArrayList<Point> list, ArrayList<Point> left_Array, int left_count, ArrayList<Point> right_Array, int right_count) {
		...
		while( i < left_count && j < right_count) {
			...
			// Sort in ascending order using y-values when y-values are equal
			else if(left_Array.get(i).y == right_Array.get(j).y) {
